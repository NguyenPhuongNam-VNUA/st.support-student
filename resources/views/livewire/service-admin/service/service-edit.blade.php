                    ...
                    {{-- <div class="mt-2">
                        @if ($new_service_galleries)
                            @foreach ($new_service_galleries as $newGallery)
                                <img src="{{ $newGallery->temporaryUrl() }}" alt="Ảnh mới của bộ sưu tập"
                                    class="img-thumbnail" width="150">
                            @endforeach
                        @endif
                    </div>
                    <div class="mt-2">
                        @if ($service_galleries && count($service_galleries) > 0)
                            @foreach ($service_galleries as $gallery)
                                <img src="{{ asset('storage/' . $gallery->image) }}" alt="Ảnh trong bộ sưu tập"
                                    class="img-thumbnail" width="150">
                            @endforeach
                        @else
                            <label class="text-danger mt-1">Chưa tải ảnh lên</label>
