    ...
    <div class="container">
                    ...
                    <div class="left-column style-1 mb-4">
                        <div class="blog-search">
                            ...
                            <form action="" wire:submit.prevent="searchPost" class="search-form">
                                <input type="input" class="text-black" wire:model.live="search"
                            ...
                            @foreach ($serviceCategories as $category)
                                <li class="grid-list-option">
                                    ...
                                    <a href="javascript:void(0)">
                                        {{ $category->name }}
                                        <span class="grid-items">
                                            ({{ $category->services->where('status', \App\Enums\StatusRequest::Completed->value)->count() }})
                                        </span>
                                    </a>
                                </li>
                            @endforeach
                                    ...
                                    <div class="header-pro">
                                        <div class="tred-pro">
                                            <div class="tr-pro-img">
                                                <a
                                                    href="{{ route('client.service-detail', ['slug' => $service->slug]) }}">
                                                    <img class="img-fluid"
        ...
        <div class = "mt-5">
            <span class="page-title">
                Hiển thị từ {{ $services->firstItem() }} đến {{ $services->lastItem() }} trong tổng số
                {{ $services->total() }} kết quả
            </span>
            ...
            <div class="page-number style-1">
                ...
                @foreach ($services->getUrlRange(1, $services->lastPage()) as $page => $url)
                    <a href="{{ $url }}"
                        class="page-link {{ $page == $services->currentPage() ? 'active' : '' }}">{{ $page }}</a>
                @endforeach
